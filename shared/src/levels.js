 ...
 */
export const XP_THRESHOLDS = Array.from({ length: MAX_LEVEL }, (_, i) => thresholdForLevel(i + 1));
...
export function xpForRaceResult(rank) {
    let xp = XP_AWARDS.compete;
    if (rank <= 3)
        xp += XP_AWARDS.podium;
    if (rank === 2)
        xp += XP_AWARDS.runner_up;
    if (rank === 1)
        xp += XP_AWARDS.winner;
    return xp;
}
 ...
 */
export function xpForTokenBonus(rank, tokens, winner_tokens) {
    if (rank === 1)
        return XP_AWARDS.token_bonus_max;
    if (winner_tokens <= 0)
        return 0;
