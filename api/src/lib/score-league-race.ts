...
// Award league points for a finished league fixture. No-op unless the race is a
// league fixture and the league still exists. Idempotent per (horse, round).
export async function scoreLeagueRace(race: Race, horses: Horse[]): Promise<void> {
  if (!race.league_id || race.league_season === undefined || race.league_round === undefined) return;
