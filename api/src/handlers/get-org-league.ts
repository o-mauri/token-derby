...
import type { GetOrgLeagueResponse } from '@token-derby/shared';
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  if (org.creator_user_id !== auth.user_id) {
    return err('NOT_ORG_OWNER', 'Only the organisation creator can view the league');
  }

  const league = await getLeague(org.org_id);
  const response: GetOrgLeagueResponse = { league };
  return ok(response);
};
