...
import type { DeleteOrgScheduleResponse } from '@token-derby/shared';
import { ORG_NAME_PATTERN } from '@token-derby/shared';
import { getOrganisationByName } from '../db/organisations.js';
import { deleteSchedule } from '../db/schedules.js';
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  await deleteSchedule(org.org_id);
  const response: DeleteOrgScheduleResponse = { ok: true };
  return ok(response);
};
