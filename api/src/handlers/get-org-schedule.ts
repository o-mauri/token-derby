...
import type { GetOrgScheduleResponse } from '@token-derby/shared';
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  if (org.creator_user_id !== auth.user_id) {
    return err('NOT_ORG_OWNER', 'Only the organisation creator can view the race schedule');
  }

  const schedule = await getSchedule(org.org_id);
  const response: GetOrgScheduleResponse = { schedule };
  return ok(response);
};
