...
import type { GetOrgRaceSettingsResponse } from '@token-derby/shared';
...
export const handler: ApiHandler = async (event) => {
  ...
  if (org.creator_user_id !== auth.user_id) {
    return err('NOT_ORG_OWNER', 'Only the organisation creator can view race settings');
  }

  const settings = await getRaceSettings(org.org_id);
  const response: GetOrgRaceSettingsResponse = { settings };
  return ok(response);
};
