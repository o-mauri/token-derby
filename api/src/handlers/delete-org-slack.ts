...
import type { DeleteOrgSlackResponse } from '@token-derby/shared';
import { ORG_NAME_PATTERN } from '@token-derby/shared';
import { getOrganisationByName, clearOrgSlack } from '../db/organisations.js';
...
export const handler: ApiHandler = async (event) => {
  ...
  await clearOrgSlack(org.org_id);
  const response: DeleteOrgSlackResponse = { ok: true };
  return ok(response);
};
