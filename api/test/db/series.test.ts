...
import { appendSeriesPoint, listSeriesPoints, listRecentSeriesPoints, SERIES_RETENTION_MS } from '../../src/db/series.js';
import { ddb, TABLE } from '../../src/db/client.js';
import { seriesPointKey } from '../../src/db/keys.js';
...
describe('series db', () => {
  ...
  it('stamps a ttl of (t + retention) in epoch SECONDS on each point', async () => {
    const race_id = rid();
    const t = 1_700_000_000_000; // fixed ms epoch
    await appendSeriesPoint(race_id, 'h1', 1, { t, d: 5 });
    const { Item } = await ddb.send(new GetCommand({
      TableName: TABLE,
      Key: seriesPointKey(race_id, 'h1', 1),
    }));
    expect(Item?.ttl).toBe(Math.floor((t + SERIES_RETENTION_MS) / 1000));
  });
