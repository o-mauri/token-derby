...
import { putUser, getUsersByIds } from '../../src/db/users.js';
...
describe('getUsersByIds', () => {
  it('maps each id to its display name', async () => {
    ...
    const names = await getUsersByIds([a, b]);
  ...
  it('returns an empty map for no ids without hitting the table', async () => {
    expect(await getUsersByIds([])).toEqual(new Map());
  });
  ...
  it('omits ids that have no user row', async () => {
    ...
    const names = await getUsersByIds([real, 'u-does-not-exist']);
  ...
  it('deduplicates repeated ids and ignores empty strings', async () => {
    ...
    const names = await getUsersByIds([a, a, '', a]);
