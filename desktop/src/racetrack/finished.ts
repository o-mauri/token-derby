// Ported from site/src/render/finished.ts. The one seam change: the site
// version defaults `fetchSeries`/`fetchStandings` to its own network fetchers
// (../api.js) when the caller doesn't supply them; here both are optional with
// NO network default — render.ts is responsible for wiring real hooks (backed
// by window.api), and if a hook is omitted the corresponding face is simply
// skipped (see mountDetailCycle below) rather than reaching for a fetch.
...
function scheduleBarAnimation(overlay: HTMLElement): void {
  // jsdom/happy-dom (used in tests) doesn't support requestAnimationFrame in
  // the same way as browsers — fall back to a microtask so the test
  // environment still sees the target width applied.
...
async function mountDetailCycle(
  ...
  // Token graph: only attempted when the caller injected a series source
  // (render.ts wires this to window.api.getRaceSeries). No hook → no chart
  // faces, rather than reaching for a real fetch.
  if (opts.fetchSeries) {
    try {
      const series = await opts.fetchSeries(race.join_code);
  ...
  // League season face: only attempted when both the race is a league fixture
  // AND the caller injected a standings source. Desktop doesn't wire one up
  // yet, so this face is simply absent there — see racetrack/render.ts.
  if (opts.fetchStandings && race.league_id && race.league_season !== undefined && race.organisation_name) {
    try {
      const { standings } = await opts.fetchStandings(race.organisation_name, race.league_season);
