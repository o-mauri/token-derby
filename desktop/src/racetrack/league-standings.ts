// Ported unchanged from site/src/render/league-standings.ts — pure DOM
// building off @token-derby/shared types, no seam changes needed. Only
// reachable in desktop when a `getLeagueStandings` hook is injected into
// renderRace/renderFinishedOverlay (see render.ts, finished.ts) — desktop
// doesn't wire that up yet, so this is currently dead code kept for parity
// and to make wiring league support later a data-hookup rather than a UI port.
