...
it('re-sends the identical snapshot on retry, re-prepares after success', async () => {
  let n = 0;
  const prepareBeat = vi.fn(async () => ({ seq: ++n, components: { claude: n * 10, codex: 0, gemini: 0 }, convReadings: { claude: {}, codex: {}, gemini: {} } }));
  const sent: any[] = [];
...
it('stops and calls onFinished when race_status is finished', async () => {
  const prepareBeat = vi.fn(async () => ({ seq: 1, components: { claude: 0, codex: 0, gemini: 0 }, convReadings: { claude: {}, codex: {}, gemini: {} } }));
...
describe('runHeartbeatLoop', () => {
  ...
  it('retries with backoff after a failure', async () => {
    let callCount = 0;
    const prepareBeat = vi.fn(async () => ({ seq: ++callCount, components: { claude: 0, codex: 0, gemini: 0 }, convReadings: { claude: {}, codex: {}, gemini: {} } }));
