...
it('re-sends the identical snapshot on retry, re-prepares after success', async () => {
  let n = 0;
  const prepareBeat = vi.fn(async () => ({ seq: ++n, components: { anthropic: n * 10, openai: 0, google: 0 }, convReadings: { anthropic: {}, openai: {}, google: {} } }));
  const sent: any[] = [];
...
it('stops and calls onFinished when race_status is finished', async () => {
  const prepareBeat = vi.fn(async () => ({ seq: 1, components: { anthropic: 0, openai: 0, google: 0 }, convReadings: { anthropic: {}, openai: {}, google: {} } }));
...
describe('runHeartbeatLoop', () => {
  ...
  it('retries with backoff after a failure', async () => {
    let callCount = 0;
    const prepareBeat = vi.fn(async () => ({ seq: ++callCount, components: { anthropic: 0, openai: 0, google: 0 }, convReadings: { anthropic: {}, openai: {}, google: {} } }));
